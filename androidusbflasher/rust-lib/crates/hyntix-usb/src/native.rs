...
use crate::config::{INITIAL_URB_CHUNK_SIZE, MIN_URB_CHUNK_SIZE, URB_PIPELINE_DEPTH};
...
impl NativeUsbBackend {
    ...
    pub fn bulk_out_with_timeout(&self, data: &[u8], _timeout_ms: u32) -> std::io::Result<usize> {
        ...
        while submit_offset < data.len() || !in_flight.is_empty() {
            ...
            while submit_offset < data.len() && in_flight.len() < URB_PIPELINE_DEPTH {
            ...
            // ── Reap phase: collect one completed URB ────────────────────
            if !in_flight.is_empty() {
                let reaped_ptr = match self.reap_urb() {
                    Ok(ptr) => ptr,
                    Err(e) => {
                ...
                // Find the reaped URB in our queue (usually front — FIFO order)
                let mut found = false;
                for i in 0..in_flight.len() {
                    ...
                    if urb_ptr == reaped_ptr as *const UsbDevFsUrb {
                        ...
                        if urb.status != 0 {
                            ...
                            // EPIPE (stall): clear halt and treat as error
                ...
                for i in 0..in_flight.len() {
                    ...
                    if urb_ptr == reaped_ptr as *const UsbDevFsUrb {
                        ...
                        if (urb.actual_length as usize) < (urb.buffer_length as usize) {
                            // Drain remaining URBs — we got less than requested
                            self.drain_urbs(&mut in_flight);
